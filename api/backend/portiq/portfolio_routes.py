...
@portfolio_routes.route("/portifolio",methods=["GET"])
def GetAllPortifolios():
  cursor = get_db().cursor(dictionary=True)
  try:
    current_app.logger.info("GET portifolii")
    ...
    cursor.execute(query,params)
    current_app.logger.info("Got the portifolios")
    portifolios = cursor.fetchall()
    return jsonify(portifolios), 200
  except Error as e:
    current_app.logger.error(f"The error {e} occured while getting all portifolios")
    return jsonify(str(e)), 500
  finally:
    cursor.close()
  

@portfolio_routes.route("/portifolio/<int:portifolio_id>",methods)
